...
function () {
  ...
  _createClass(ServiceCollection, [{
    ...
    value: function _push(service, desc) {
      ...
      if ((0, _util.isFunction)(service.setup)) service.setup(this.container, this.trimDesc(desc));
    }
  }, {
    key: "add",
    ...
    value: function get(name) {
      var service = this.services[this.names[name]],
          parent = this.container.parent; // If parent exist, get from there instead
    ...
    key: "trimDesc",
    value: function trimDesc(service) {
      return {
        name: service.name
      };
    }
