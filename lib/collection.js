...
function () {
  ...
  _createClass(ServiceCollection, [{
    key: "_push",
    value: function _push(value, desc, skipIfExist) {
      var name = desc.name = (desc.name || value.service || '').toLowerCase();
      ...
      if (index > -1) {
        ...
        if (value.singleton && desc.type !== this.types.SINGLETON) throw new Error("Service \"".concat(name, "\" must be a singleton"));
      ...
      if ((0, _util.isFunction)(value.setup)) {
        var core = this.services[0].value();
