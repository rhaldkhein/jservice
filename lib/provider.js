...
function () {
  ...
  _createClass(ServiceProvider, [{
    ...
    value: function _createService(name) {
      ...
      if (service === undefined) return null; // Validate resolution, singleton must not resolve scoped or transient.
    ...
    value: function _instantiate(service) {
      ...
      if (!(0, _util.isObject)(objConfig)) throw new Error('Service\'s config must be object');
      objConfig.name = service.name;
      return (0, _util.isConstructor)(Service) ? new Service(provider, objConfig) : Service(provider, objConfig);
    }
  }]);
